...
router.route('/')
  ...
  .delete(async (req, res) => {
    ...
    try {
      ...
      await User.updateMany({ friends: { $in: [id] } }, { $pull: { friends: id } })
      await User.updateMany({ peopleIdontWannaSeeAgain: { $in: [id] } }, { $pull: { peopleIdontWannaSeeAgain: id } })
...
router.route('/peopleIdontWannaSeeAgain')
  .patch(async (req, res) => {
    ...
    console.log("---------peopleIdontWannaSeeAgain", req.session?.me?._id, "console.log", peopleIdontWannaSeeAgainOneUser);

    try {
      const updatePeopleIdontWannaSeeAgain = await User.findByIdAndUpdate(id, { $push: { peopleIdontWannaSeeAgain: peopleIdontWannaSeeAgainOneUser } }, { new: true });

      if (!updatePeopleIdontWannaSeeAgain) {
        return res.status(404).json({ error: "document not found" })
      }
      req.session.me.peopleIdontWannaSeeAgain.push(peopleIdontWannaSeeAgainOneUser)
...
router.route('/updateArrayFriends')
  .patch(async (req, res) => {
    ...
    try {
      const updateArrayWithMyFriendsId = await User.findByIdAndUpdate(id, { $push: { friends: newFriendId } }, { new: true });
...
router.route("/updateImage")
  .put(async (req, res) => {
    ...
    console.log("id", id)
    try {
      let updatePic = await User.findByIdAndUpdate(id, { image: newImage }, { new: true });
      if (!updatePic) {
...
router.route("/updateName")
  .put(async (req, res) => {
    ...
    try {
      ...
      const updatedName = await User.findByIdAndUpdate(id, { name: name }, { new: true })
      if (!updatedName) {
        res.status(500)
...
router.route("/updateLocation")
  .put(async (req, res) => {
    ...
    try {
      const updateLocation = await User.findByIdAndUpdate(id, { location: location }, { new: true })
      if (!updateLocation) {
        res.status(500)
